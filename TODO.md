...
- [ ] **R4 — re-verify the 4 STFT "criticals" (A29-C1/C4/C5/C6).** Marked blocking in `REVIEW-AUDIO.md` but predate the harmonic-tracker rework; could be stale or still real. Re-check each against current code via `stft-validator` before treating as open blockers. *Size: M.*
...
- [ ] **R6 — retro-tag v1.2.0/v1.3.0/v1.3.1 on the mirror; set a tag cadence.** Tags stop at v1.1.0 though three releases shipped in CHANGELOG. *Depends on R1. Size: S.*
- [ ] **R7 — triage the ~20 open A29-H items.** Confirm which survive the harmonic-tracker rework; close stale ones so the backlog stays credible. *Size: M.*
- [ ] **R8 — warning-reduction pass.** 272 warnings (256 `-Wsign-conversion`) bury real ones. Targeted fixes or scoped suppression. *Size: M.*

### Nice to have

- [ ] **R9 — UX: default state passes audio unchanged / blank spectrum** (tracks C5/N1). "Is it working?" first impression. *Size: S–M.*
