...
- Added user preset save: Save/Rename/Delete a preset from the Presets menu, alongside the
  ...
  Windows and Linux, and the current preset name is shown beside the menu.
...
Closes the actionable findings of three internal review passes (QA, UX, and a second design pass).

- **A single NaN/Inf input sample can no longer kill the output permanently** (QA-C1/C2). Input is scrubbed at both processor entries (non-finite → 0, one branch per sample), and defense-in-depth: the mask-estimator's transient-envelope recurrence is now self-healing (`clamp01` NaN-safe, non-finite state resets) — a DSP-layer Inf that previously poisoned the output *forever* now bounds to ~75 ms.
- **The Bypass parameter now bypasses the brightness EQ too** (QA-H1: it measured +11.3 dB at 8 kHz while "bypassed"; the parameter route and the host bypass route now agree).
- **No more fade-in from silence after prepare** (QA-H2): gain smoothers seed at unity on construct/prepare/reset and snap to the real parameter values in prepareToPlay — an impulse in the very first block of an offline bounce now survives at full amplitude.
- **Separation quality is no longer sample-rate dependent** (QA-M1). Two root causes: `initializeComponents()` silently dropped the spectral floor on every prepare (one-line fix), and per-frame time constants (mask smoothing, transient envelope, median depth, low-freq tracker confirm counts) halved their time windows at high rates — all now normalized to seconds. Hum rejection at the noise corner: 88.2 kHz −39.7 dB (failed the gate) → **−68.7 dB**; every rate 44.1k–192k now −54…−75 dB. New permanent harness gate sweeps 44.1/88.2/96/192 kHz.
- **Oversized host blocks process correctly in chunks** (QA-M2, was silent corruption), **zero-sample blocks are legal** (VST3 flushes), and **processBlock after releaseResources outputs silence** instead of leaking dry input (QA-M3).
- **The XY pad no longer silently rides the transient stream** (internal UX review finding 1). The pad-corner coupling got a knee (`1 − (1−m)^16`): working moves leave transients alone (−20 dB de-noise now costs consonants −1.8 dB effective instead of −20), exact corners still silence bleed. A small **"eff" readout under the TRANS fader** surfaces the effective gain whenever it differs from the setting — the fader never lies.
- **Presets deliver their names** (internal UX review finding 2, re-measured on the dialogue scene): Transient Punch actually punches (consonants +3.7 dB), Ambience Rescue keeps the bed (−0.2 dB), Dialogue De-noise keeps consonants; **Gentle Separation now audibly does something** (mild −6 dB de-noise — it was bit-identical to the input).
- **Steady noise no longer routes into the Transient stream** (internal UX review finding 7 / backlog A29-H9). Root cause: steady broadband beds have persistently high per-bin spectral flux, so the transient envelope treated hiss as endless onsets. A frame-level onset gate (mean rising flux vs. its own slow baseline) now opens the flux→transient path only on real onsets. Measured on the dialogue scene: pad de-noise at −20 dB now removes **−19.2 dB** of hiss (was −6.0 — the removed noise used to leak back through the open transient stream); the TRANS fader no longer doubles as a hiss fader (+12 dB during a de-noise leaves hiss at −19.2, was +5.3); Tame Transients leaves the steady bed untouched (−0.0, was −5.7); Dialogue De-noise reaches −18.0 dB (was −8.4); and **Extract Noise now returns the bed at full level** (+0.0, was −6.7 — closes internal UX review finding 6). Two harness noise-floor rows moved by ~3 dB (sine corner −146.4 → −143.2, 192k hum −75.2 → −71.9) — control-experiment-verified as energy the old bug had been hiding in the corner-muted transient stream; both remain 32–93 dB inside their targets.
- **Mojibake fixed** ("Waiting for audioâ€¦", the XY-pad zoom hint's "â€¢", and two tooltip em-dashes) — all UI literals ASCII-safe.

### Changed (design pass 2 fixes, 2026-07-03)

- Bipolar knobs (FOCUS, BRIGHT) draw **center-anchored arcs**: neutral reads as empty, not half-lit (D2-1).
- **Keyboard focus is visible and Tab works in the standalone** (D2-2): the window itself was taking focus, so traversal had no starting point; the editor now seeds the first control when the window is key (never steals from a real control; DAW behavior unchanged — click into the editor and Tab from there). Live-verified: ring on BYPASS at launch, Tab moves it.
- **Spectrum ribbon is post-gain** (D2-6): a muted/attenuated stream thins or vanishes in the ribbon instead of pulsing at full detection strength.
- Solo-on color is now the accent teal (tokenized `Theme::soloOn`) — a lit SOLO no longer masquerades as the transient stream's yellow (D2-3); mute-on red tokenized alongside.
- **LOG/LIN toggle moved onto the spectrum it controls** (D2-5); redundant "PRESET" caption removed (D2-8); preset combo width capped and **the spectrum grows with the window** instead of staying an 80 px sliver (D2-4).
...
Audit-fix pass against the post-v1.3.0 tree (from an internal audit review and roadmap, 2026-05-29 entries). Distribution and host-integration correctness; STFT-knot DSP fixes (A29-C1/C4/C5/C6) intentionally deferred to a follow-up PR gated on the `stft-validator` agent.
...
- **Third audio stream: Transient.** The plugin is now a true 3-stream decomposition (Tonal + Transient + Noise), inspired by the same conceptual structure as the Deconstruct module in iZotope RX. Drum hits, plosives, consonant onsets, and other short broadband events now route to their own stream — independently gainable, soloable, and muteable — instead of being mixed into the Noise output. Workflows this enables that were impossible before:
