...
- **`spectralFlux` now detrends against the running per-bin median (`horizontalGuide`) instead of the previous frame.** This is the upstream cause of the "fully noise still holds tonality" symptom on pitched modulated material (drones with vibrato, sustained hums with audible buzz, lightsaber-style sounds). With the previous frame-to-frame measure, a steady drone with ±30 % tremolo registered flux ≈ 0.33 on every harmonic bin; the flux penalty then trimmed `tonalPower` by ~23 %, propagating through the Wiener mask and the three-stream split, so drone harmonics got routed into the Noise stream. At the XY pad's "fully noise" corner that energy received +12 dB amplification — leaving the tonality audible instead of silenced. Detrending against the per-bin median makes AM/tremolo invisible to flux (the median averages the modulation out), while real onsets still spike above the median in 1-2 frames and register normally. After this change, modulated drones classify as tonal again and the corners isolate symmetrically. (User feedback continuation, 2026-05-29 — supersedes the parameter-tuning attempts that didn't address the root cause.)
- **Mask split is now sharpened post-3-stream so "fully noise" actually silences tonal content (and vice versa).** The Wiener step in `MaskEstimator` is `pow(wienerGain, maskExponent)`; because `wienerGain ≤ 1`, the resulting tonal mask peaks around 0.8 even for a clearly-tonal drone bin. The leftover ~0.2 leaks into noise/transient, so the gain corners were asymmetric: the dominant stream's gain produced a dramatic boost (+10 dB or more) while the orthogonal gain only produced a modest attenuation (~−5 dB) on the same bin — enough that pushing "fully noise" on a lightsaber-style sound left audible tonality. After the three-stream split, the masks are now sharpened (`m_i' = m_i^2 / Σ m_j^2`) so dominant bins push close to 1.0 and orthogonal ones close to 0. Mass-conservation preserved (sum stays 1). At a perfectly balanced bin (1/3 each) the operation is a no-op. Empirically a typical drone bin's tonal mask goes 0.81 → 0.97, the noise share drops ~0.15 → ~0.03, and "fully noise" then attenuates the drone by about −27 dB instead of −5 dB. Symmetric for "fully tonal" on noise-dominant material. (User feedback, 2026-05-29.)
